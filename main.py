...
DB_PATH = str(BASE_DIR / "barber.db")
...
def admin_create_designer(payload: AdminDesignerCreate):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute(
        """
...
def admin_update_designer(designer_id: int, payload: AdminDesignerUpdate):
    conn = get_db()
    cursor = conn.cursor()
...
def admin_delete_designer(designer_id: int):
    conn = get_db()
    cursor = conn.cursor()
    existing = cursor.execute(
        "SELECT id FROM designers WHERE id = ?",
...
def admin_create_service(payload: AdminServiceCreate):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute(
        """
...
def admin_update_service(service_id: int, payload: AdminServiceUpdate):
    conn = get_db()
    cursor = conn.cursor()
...
def admin_update_business_hours(record_id: int, payload: AdminBusinessHoursUpdate):
    conn = get_db()
    cursor = conn.cursor()
    existing = cursor.execute(
...
def admin_cancel_booking(booking_id: int, payload: Optional[AdminBookingCancel] = None):
    _ = payload
